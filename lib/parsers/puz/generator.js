...
function writeExtensions(answerArray, userSolutionArray, timing) {
    ...
    const grbsBuffer = writeExtension(new Buffer(answerArray.map((cell, index) => {
        const key = findKey_1.default(rebusSolutions, (solutionInfo) => solutionInfo.cells.includes(index));
        if (key === undefined) {
            return 0;
        }
...
function magicChecksum({ header, answer, solution, title, author, copyright, notes, clueList, }) {
    ...
    const answerChecksumValue = doChecksum(iconv_lite_1.default.encode(answer, puz_reader_1.default.ENCODING));
    const solutionChecksumValue = doChecksum(iconv_lite_1.default.encode(solution, puz_reader_1.default.ENCODING));
...
exports.generate = (puzzle, solutionKey) => {
    ...
    const userSolution = puzzle.userSolution.map((row) => row.map(
    ...
    const userSolutionStringBuffer = iconv_lite_1.default.encode(userSolutionArray.map(
    // tslint:disable-next-line:no-shadowed-variable
    (solution) => solution[0]).join(""), puz_reader_1.default.ENCODING);

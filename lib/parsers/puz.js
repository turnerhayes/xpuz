...
function _textChecksum(puzzleData, checksum) {
	...
	var versionParts = puzzleData.header.version.split(".").map(Number);
...
function _writeHeader(puzzleData, options) {
	...
	return Buffer.concat([globalChecksumBuffer, iconv.encode("ACROSS&DOWN\0", PUZReader.ENCODING), headerChecksumBuffer, magicChecksumBuffer, iconv.encode(get(options, "version", puzzleData.header.version) + "\0", PUZReader.ENCODING),
	// unknown block 1

...
describe("Puzzle object", function() {
	...
	it("should clone to a new Puzzle instance", function() {
		...
		expect(cloned.extensions, "Extensions in cloned").to.deep.equal(puzzle.extensions);
		expect(cloned.extensions, "Extensions in cloned").to.not.equal(puzzle.extensions);
	});
	...
	it("should correctly determine equality", function() {
		...
		otherPuzzle.grid[0][1].solution = "T";
	...
	it("should place clue numbers in the appropriate cells", function() {
		...
		/* eslint-disable no-magic-numbers */
		expect(puzzle.grid[0][1].clueNumber).to.equal(1);
		expect(puzzle.grid[0][2].clueNumber).to.equal(2);
		expect(puzzle.grid[1][0].clueNumber).to.equal(3);
		expect(puzzle.grid[2][0].clueNumber).to.equal(4);
		/* eslint-enable no-magic-numbers */
	});
	...
	describe("updateGrid", function() {
		it("should update the grid", function() {
			...
			expect(puzzle.grid[1][0].clueNumber, "Clue number at [0, 1] after change").to.equal(3);
			expect(puzzle.grid[1][0].containingClues, "Containing clues at [0, 1] before change").to.deep.equal({
				down: 3,
			});
			// eslint-disable-next-line no-magic-numbers
			expect(puzzle.grid[2][0].clueNumber, "Clue number at [0, 2] before change").to.equal(4);
			expect(puzzle.grid[2][0].containingClues, "Containing clues at [0, 2] before change").to.deep.equal({
				across: 4,
				down: 3
			});
			...
			expect(puzzle.grid[1][0].clueNumber, "Clue number at [0, 1] after change").to.be.undefined;
			expect(puzzle.grid[1][0].containingClues, "Containing clues at [0, 1] after change").to.be.undefined;
			// eslint-disable-next-line no-magic-numbers
			expect(puzzle.grid[2][0].clueNumber, "Clue number at [0, 2] after change").to.equal(3);
			expect(puzzle.grid[2][0].containingClues, "Containing clues at [0, 2] after change").to.deep.equal({
				across: 3,
			});
		});
	});

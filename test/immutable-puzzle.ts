...
describe("ImmutablePuzzle object", function() {
	...
	it("should correctly determine equality", function() {
		...
		expect(
			puzzle.equals({ not: "a puzzle"} as any),
			"Compare against plain object"
		).to.be.false;
		expect(puzzle.equals(mutablePuzzle as any), "Compare against mutable puzzle").to.be.false;
		expect(puzzle.equals(null as any), "Compare against null").to.be.false;

		otherPuzzle = otherPuzzle.setIn(["grid", 0, 1, "solution"], "T") as ImmutablePuzzle;

		expect(puzzle, "Compare against different puzzle").not.to.equal(otherPuzzle);
	});
	...
	it("should update clue numbers and containing clues when the grid is changed", function() {
		...
		puzzle = puzzle.setIn(["grid", 1, 0], Map({ isBlockCell: true })) as ImmutablePuzzle;
